...
async function cargarObrasPorPrecioAsc()  {
    const query = `{ ObrasPorPrecio(limit:150, offset:0) { id nombre precio foto artista { id nombre } genero { id nombre } } }`;
    try {
        const res = await fetch("http://localhost:8080/query", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query }) });
        const data = await res.json();
...
async function cargarObrasPorPrecioDesc() {
    const query = `{ ObrasPorPrecioDesc(limit:150, offset:0) { id nombre precio foto artista { id nombre } genero { id nombre } } }`;
    try {
        const res = await fetch("http://localhost:8080/query", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query }) });
        const data = await res.json();
...
async function cargarObrasPorGenero(idGenero) {
    ...
    const query = `{ ObrasPorGenero(idGenero: ${idGenero}, limit:150, offset:0) { id nombre precio foto artista { id nombre } genero { id nombre } } }`;
    try {
        const res = await fetch("http://localhost:8080/query", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query }) });
        const data = await res.json();
...
async function cargarObrasPorDisponibilidad() {
    const query = `{ ObrasPorDisponibilidad(limit:150, offset:0) { id nombre precio foto status artista { id nombre } genero { id nombre } } }`;
    try {
        const res = await fetch("http://localhost:8080/query", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query }) });
        const data = await res.json();
...
async function cargarFiltros() {
    const artistasQuery = `query { Artistas(limit:150, offset:0) { id nombre } }`;
    const generosQuery = `query { Genero(limit:150, offset:0) { id nombre } }`;
